...
and shared cursor, then disappears automatically when the primary map returns.
...
is unavailable instead of accumulating an incomplete alias list. Go Live uses one
validated HTTP or HTTPS service endpoint for session creation, realtime publishing,
spectator connections, and shareable links. Hosted installs default to
`https://live.simtrace.run`; self-hosters can replace it with their compatible service.
