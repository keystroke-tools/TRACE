...
export function SetupsPage() {
	...
	const [workspace, setWorkspace] = useState<"library" | "import">("library");
	...
	async function importArchives(paths: string[]) {
		...
		try {
			...
			if (nextResults.some((result) => result.success && !result.indexWarning)) setLibraryVersion((value) => value + 1);
	...
	async function indexExisting() {
		...
		try {
			...
			if (result.indexed > 0) setLibraryVersion((value) => value + 1);
			showToast({
	...
	return (
		<>
			<PageIntro
				...
				eyebrow="CAR SETUPS"
				title="SETUP LIBRARY"
				description="Browse setups across supported simulators, or import new files into the layout each simulator expects."
			/>
			<div className="mt-7 grid grid-cols-2 border border-trace-divider bg-trace-surface" role="tablist" aria-label="Setup workspace">
				{(
					[
					...
					<button
						...
						className={`h-12 border-b-2 font-mono text-[11px] font-black tracking-[.1em] ${workspace === tab.id ? "border-trace-accent bg-trace-deep text-white" : "border-transparent text-trace-dim hover:bg-trace-deep hover:text-white"}`}
						key={tab.id}
					>
						{tab.label}
					</button>
			...
			{workspace === "library" ? (
				<SetupExplorer refreshKey={libraryVersion} />
			) : (
				<>
					...
					<p className="mt-4 max-w-4xl text-[11px] leading-5 text-trace-dim">
						Assetto Corsa is the first setup-import adapter. Imported setups are indexed for exact session matches; open a session to mark the setup
						you used, compare its INI values, and include it in a shared .trace package. TRACE cannot automatically identify which setup was active
						while you drove.
					</p>
				</>
			)}
