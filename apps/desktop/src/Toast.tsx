...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={showToast}>
      {children}
      <div className="pointer-events-none fixed bottom-6 right-6 z-[110] flex w-[min(380px,calc(100vw-48px))] flex-col gap-2" aria-live="polite" aria-atomic="false">
        {toasts.map((toast) => <Toast toast={toast} onRemove={removeToast} key={toast.id} />)}
      </div>
...
function Toast({ toast, onRemove }: { toast: ToastRecord; onRemove: (id: number) => void }) {
  ...
  const accent = toast.kind === "success"
    ? "border-l-trace-accent"
    : toast.kind === "error"
      ? "border-l-trace-warning"
      : "border-l-trace-purple";
  ...
  return (
    ...
    >
      ...
      {toast.timeoutMs !== null && !leaving && (
        <span className={`trace-toast-progress absolute bottom-0 left-0 h-px ${toast.kind === "success" ? "bg-trace-accent" : toast.kind === "error" ? "bg-trace-warning" : "bg-trace-purple"}`} style={{ animationDuration: `${toast.timeoutMs}ms` }} aria-hidden="true" />
      )}
    </div>
  );
}
