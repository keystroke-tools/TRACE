...
LLM. TRACE uses deterministic, tested mathematics for normalization, lap delta,
corner analysis, comparisons, confidence, and evidence. LLM coaching and provider
integrations are explicitly outside the current product scope.
...
corner analysis, portable sessions, and a simulator-aware setup importer with exact
session matching, explicit setup linking, portable confirmed setups, and literal AC
INI comparison. Go Live is the active product priority. Automatic active-setup capture
and setup recommendations remain later work and do not block the live publishing and
spectator path.
